...
For playback, join a voice channel and run `/play query:<song or URL>`. Only one resolved track is accepted per command; playlist results are intentionally rejected in this phase. When a track ends, the bot starts the next item in that server's FIFO queue. `/stop` clears the in-memory queue and disconnects the bot from voice.
