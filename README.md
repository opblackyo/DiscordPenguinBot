...
DiscordPenguinBot 是一個模組化的私人 Discord 控制中心。Phase 1D 在既有音樂 MVP 上加入繁體中文回覆與 Discord Embed 播放資訊美化。
...
- Music MVP：`/play query` 會搜尋一首曲目、加入呼叫者所在語音頻道並在閒置時開始播放；`/queue`、`/nowplaying`、`/skip`、`/stop` 會操作各 guild 獨立的 queue。播放回覆使用繁體中文 Embed，顯示歌曲、作者、長度、點歌者與可用的來源連結。Lavalink 未連線時，`/play` 會安全拒絕而不影響 `/ping`。
- Tests：smoke test 會確認所有 Phase 1C 指令已註冊；playback tests 驗證 FIFO、per-guild isolation、失敗跳過與 stop 清理。

刻意未實作：Bilibili、pause/resume、loop/shuffle、playlist、lyrics、queue persistence、Dashboard 控制與 AI。這些功能不屬於 Phase 1D。
